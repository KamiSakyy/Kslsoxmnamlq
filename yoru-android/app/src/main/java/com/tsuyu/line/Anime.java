...
public final class Anime {
    public String source=Sec.s("271b1c121c3e0a111b"), id="", title="", original="", alias="", poster="", description="", type="Аниме", status="", age="", studio="", trailerUrl="", hentaUrl="", airedDate="", nextEpisodeAt="", countries="", cast="", crew="", ratings="", franchise="";
    ...
    public boolean metadataOnly(){return Sec.s("271b1c121c3e0a111b").equals(source);}
    public String meta(){String s=year>0?String.valueOf(year):"";if(!type.isEmpty())s+=(s.isEmpty()?"":" · ")+type;return s;}
    public int aired(){if(episodesAired>0)return episodes>0?Math.min(episodes,episodesAired):episodesAired;int playable=0;for(Episode e:episodeList)if(e!=null&&!e.future)playable++;int n=playable;if(episodes>0&&n>episodes)n=episodes;return Math.max(0,n);}
    ...
    public String episodesLine(){int a=aired();if(a<0)a=0;int total=episodes;boolean known=total>0;if(finished()&&a>0){total=a;known=true;}if(known&&a>0&&a<total)return "Вышло "+a+" из "+total;if(known)return total+" "+seriesPlural(total);if(a>0)return "Вышло "+a;return "";}
