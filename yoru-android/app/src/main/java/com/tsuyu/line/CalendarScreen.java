...
public final class CalendarScreen extends FrameLayout {
    ...
    private int kindColor(ApiRepository.AiringItem item){if(item==null)return Ui.PURPLE;if("Премьера".equals(item.kind))return Ui.AMBER;if(item.anime!=null&&item.anime.episodesAired>0){if(item.anime.episodesAired>=item.episode)return Ui.EMERALD;return Ui.PURPLE;}if(item.time<System.currentTimeMillis())return Ui.EMERALD;if(item.anime!=null&&item.anime.ongoing())return Ui.PURPLE;return Ui.ZINC4;}
