...
public final class CalendarScreen extends FrameLayout {
    ...
    private void initCached(){final int gen=generation;YoruApp.app().io.execute(()->{try{ArrayList<ApiRepository.AiringItem> raw=ApiRepository.airingFromJson(YoruApp.app().cache.schedule(24*60*60*1000L));final ArrayList<ApiRepository.AiringItem> cached=raw.isEmpty()?ApiRepository.airingFromJson(readLegacyCache()):raw;if(!cached.isEmpty()){final BucketData d=computeBuckets(cached);YoruApp.app().main.post(()->{if(!alive()||gen!=generation||!items.isEmpty())return;items.clear();items.addAll(cached);buckets.clear();buckets.putAll(d.buckets);filterCounts.clear();filterCounts.putAll(d.counts);bucketsDirty=false;rebuildVisible(true);adapter.publish();});}}catch(Exception ignored){}});}
