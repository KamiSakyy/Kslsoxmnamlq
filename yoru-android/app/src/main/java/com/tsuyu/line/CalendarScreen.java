...
public final class CalendarScreen extends FrameLayout {
    ...
    private final SimpleDateFormat timeFormat=new SimpleDateFormat("HH:mm",ru);
    private final SimpleDateFormat shortDayFormat=new SimpleDateFormat("EEE dd.MM",ru);
    private final SimpleDateFormat longDayFormat=new SimpleDateFormat("EEEE, dd MMMM",ru);
    ...
    private void growVisible(){if(visible.size()>=allRows.size())return;if(layout.findLastVisibleItemPosition()<adapter.getItemCount()-2)return;int oldSize=visible.size();shownCount=oldSize+24;int total=allRows.size(),end=Math.min(shownCount,total);if(end>oldSize){visible.addAll(allRows.subList(oldSize,end));shownCount=end;String tail=end<total?" из "+total:"";state=selected<0?"Все события · "+end+tail:longDays[Math.max(0,Math.min(DAYS-1,selected))]+" · "+end+tail;if(generalUnavailable)state+=" · общее расписание недоступно, показана доступная часть";adapter.append(oldSize,end-oldSize);}}
    ...
    private String countdown(ApiRepository.AiringItem item){if(item==null)return "";long t=item.time;long diff=t-System.currentTimeMillis();if(diff>0){long min=diff/60000,h=min/60,d=h/24;if(d>0)return "через "+d+" д "+(h%24)+" ч";if(h>0)return "через "+h+" ч "+(min%60)+" мин";return "через "+Math.max(1,min)+" мин";}boolean hasAired=item.anime!=null&&(item.anime.finished()||(item.anime.episodesAired>0&&item.anime.episodesAired>=item.episode));if(hasAired)return "уже вышло";if(diff>=-2*60*60*1000L)return "выходит сейчас";return "ожидается выход";}
    ...
    private final class CalendarAdapter extends RecyclerView.Adapter<CalendarHolder> {
        ...
        void append(int startIndex,int count){
            for(int i=startIndex;i<startIndex+count;i++)rows.add(new CalendarEntry(1,visible.get(i)));
            notifyItemRangeInserted(startIndex+1,count);
            notifyItemChanged(0);
        }
