...
public final class YoruApp extends Application {
    ...
    private static ExecutorService ioPool(){int cores=Math.max(2,Runtime.getRuntime().availableProcessors());return pool("yoru-io",Math.max(4,Math.min(8,cores)),Math.max(10,Math.min(16,cores*3)),Thread.NORM_PRIORITY);}
    private static ExecutorService uiPool(){int cores=Math.max(4,Runtime.getRuntime().availableProcessors());return pool("yoru-ui",Math.max(4,Math.min(6,cores)),Math.max(10,Math.min(16,cores*3)),Thread.NORM_PRIORITY+1);}
